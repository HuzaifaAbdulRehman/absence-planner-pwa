...
function parseCellEntry(cellText, room, day, slotNumber) {
  if (
    !cellText ||
    cellText.trim() === "" ||
    cellText.toLowerCase().includes("reserved")
  ) {
    return null;
  }
    ...
    for (let slot = 1; slot <= 9; slot++) {
      ...
      if (entry) {
        ...
        if (isLikelyMultiSlot) {
          ...
          const endTime =
            TIME_SLOTS[endSlot]?.split("-")[1] ||
            TIME_SLOTS[slot]?.split("-")[1];
